...
- Custom/wildcard routes: first decided **not supported**, then **reversed the same day** once it turned out kempo core serves its entire API through the wildcard `"/kempo/**": "../node_modules/kempo/dist/kempo/**"`. Upgrades now resolve exact custom route, then wildcard, then the served tree (HTTP order), and refuse any path with a `..` segment because `WS.js` is executed.
...
- 357 node tests pass, and the masked-failure audit reports zero (see the corrections below). Browser-verified via Chrome DevTools against `npm run dev`: connect, text echo, binary echo with bytes 250/255 intact, reconnect after reload, and an HTTP POST pushing to the connected page. SIGINT verified to deliver close 1001 and exit 0 (had to be triggered in-process — Windows cannot deliver a catchable SIGINT to a child).
- A live demo ships at `docs-src/websocket-demo.page.html` with `docs-src/echo/WS.js` and `docs-src/echo/push/POST.js`.

**Released (2026-09-25): kempo-server 3.4.0**, merged to `main` (`69403c0`) with the changelog closed out (`33f68a8`, both `[skip ci]` so the push did not also publish a patch), then published via the manual `minor` workflow run. CI built the project and ran the full suite on Linux before publishing: 356 of 356. Verified against the published package from a clean project: `kempo-server/websocket` resolves by package name, echo and an HTTP-route push work, a `WS.js` mapped in from `node_modules` through `customRoutes` connects, and a path with no `WS.js` is refused. 3.4.0 also carries the previously unpublished templating work, and it satisfies kempo core's `kempo-server >= 3.4.0` peer, which was unmet on the registry until now. npm took about four minutes to show the new version after the workflow reported success.

**Correction (2026-09-25): the first round of tests could not fail.** The repo's test harness keeps whichever of `pass()`/`fail()` was called last, and my integration tests did `return fail(...)` from inside a `withTestDir` callback and then fell through to `pass(...)`, so every failure was reported as a pass. The "all 17 pass" claim in the first pass was therefore not evidence. Once the tests threw on failure, three real failures surfaced, two of them genuine bugs in the implementation:
- **Half-open socket leak.** A client that hangs up cleanly without a WebSocket close frame left its socket, timers and registry entry behind. HTTP server sockets are half-open, so the server only sees `end` and never `close` unless it ends its own side. This broke the "no socket or timer is leaked after a client vanishes" criterion. Fixed by handling `end`; the route now sees close code 1006.
- **Programmatic `server.close()` never sent 1001.** I had listened for the server's `close` event, but `net.Server` only emits that once every connection has ended, and an open socket is such a connection. Fixed by wrapping `server.close`, scoped so closing one server does not close another's sockets. (The CLI SIGINT path was unaffected, since it closes sockets explicitly.)
- The third failure was a wrong test expectation (a missing `Upgrade` header is not an upgrade request at all).
Each fix now has a test that fails when the fix is removed, checked by mutation. The repo's existing tests that use the same `return fail()`-then-`pass()` shape have the same weakness; not touched.

**Second correction (2026-09-25): middleware was being handed the wrong objects, and it would have broken every kempo handshake.** The upgrade path ran the middleware chain with the raw `IncomingMessage` and a bare `ServerResponse`. Built-in middleware tolerates that, which is why the first round of tests passed. Custom middleware does not: kempo's reads `request.path` and `request.cookies` and calls `response.redirect()`, all of which come from the enhanced wrappers, so it threw and the handshake was killed. Fixed by building the enhanced request and response once, up front, exactly as the HTTP path does. The route now also receives that same request object rather than a fresh one, because data a middleware attaches to the request (a user, a session) was otherwise silently discarded. A hand-made `ServerResponse` is also never closed by the HTTP server, so a rejected upgrade left its connection open; the socket is now ended once the response finishes. New `tests/websocket-kempo-layout.node-test.js` reproduces a kempo install (wildcard-mapped API under `node_modules`, kempo-style config, a stand-in for its middleware) and fails without each of these three fixes, checked by mutation.

**Found while auditing the test suite (outside this task's code, fixed on this branch):** five existing tests were failing while reporting PASS, via the same masked-failure pattern. Three were real product or test bugs: `maxRescanAttempts` performed one rescan too many (fixed), a fixture (`late.html`) had drifted so a test could never see the 404 it expected, and two tests compared against LF in a CRLF checkout. All 539 `return fail(...)` calls were converted to `throw`, and `tests/test-hygiene.node-test.js` now fails the suite if the pattern comes back. **Fixed upstream (2026-09-25):** kempo-testing-framework 1.5.0 treats `fail()` as final, so a later `pass()` is ignored and logged as ignored. kempo-server now requires `^1.5.0` (its lockfile had it on 1.4.7, the buggy runner). The `test-hygiene` guard was removed as an obsolete workaround; the `throw` conversions stay. Verified by planting the masked pattern in a throwaway test: it now reports FAIL.
