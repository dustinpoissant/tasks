...
**Found while auditing the test suite (outside this task's code, fixed on this branch):** five existing tests were failing while reporting PASS, via the same masked-failure pattern. Three were real product or test bugs: `maxRescanAttempts` performed one rescan too many (fixed), a fixture (`late.html`) had drifted so a test could never see the 404 it expected, and two tests compared against LF in a CRLF checkout. All 539 `return fail(...)` calls were converted to `throw`, and `tests/test-hygiene.node-test.js` now fails the suite if the pattern comes back. Recommended upstream: kempo-testing-framework should treat `fail()` as sticky so a later `pass()` cannot overwrite it; that would remove the whole class of bug.
