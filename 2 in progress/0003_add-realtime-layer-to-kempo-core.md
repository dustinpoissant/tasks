...
- [x] Released through kempo's normal release process (kempo 4.3.0, kempo-server 3.5.0, 2026-09-25).
...
**Released 2026-09-25.** kempo-server 3.5.0 (the transport hardening) first, then kempo 4.3.0, both as manual minors after `[skip ci]` merges. The first kempo run failed at `npm ci`: the lockfile still pinned kempo-server 3.3.0 against the `>=3.4.0` peer range, which had never been caught because nothing had installed from the lock since 3.4.0 shipped. A lock-only update (a four-line diff) fixed it and the rerun published.
